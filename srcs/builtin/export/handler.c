...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/16 22:57:19 by imarushe          #+#    #+#             */
/*   Updated: 2022/02/16 23:00:01 by imarushe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	modify(char **args, char *st, size_t i)
{
	...
	st = ft_substr(args[i], 0, ft_get_char_by_index(args[i], '=') + 1);
	if (!st)
		return ;
	end = ft_substr(args[i], ft_get_char_by_index(args[i], '=') + 1,
			ft_strlen(args[i]));
	if (!end)
	{
		free(st);
