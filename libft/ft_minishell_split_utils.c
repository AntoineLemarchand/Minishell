...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/16 14:04:12 by imarushe          #+#    #+#             */
/*   Updated: 2022/02/16 14:07:45 by imarushe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_write_words_minishell(char const *s, int words, t_split *sp)
{
	...
	while (s && s[sp->j] && ++sp->i < words)
	{
		...
		while (s[sp->j] && s[sp->j] == sp->c)
			sp->j++;
		while ((s[sp->j] && (s[sp->j] != sp->c || (s[sp->j]
						== sp->c && sp->j > 0 && s[sp->j - 1]
						== '\\'))))
		{
			ft_write_words_util(s, sp);
			if (sp->j && s[sp->j - 1] != '\\' && (s[sp->j]
					== sp->c || s[sp->j] == '\0'))
				break ;
			if (sp->j == (int)ft_strlen(s))
				break ;
